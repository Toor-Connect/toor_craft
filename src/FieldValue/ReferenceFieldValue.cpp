...
bool ReferenceFieldValue::setValueFromString(const std::string& value, std::string& error)
{
    ...
    if (!schema_.getReferencedEntityType().empty() &&
        entity->getSchema().getName() != schema_.getReferencedEntityType()) {
        error = "Referenced entity type mismatch, expected '" + schema_.getReferencedEntityType() + "'";
        return false;
    }
    referencedId_ = value;
    return true;
